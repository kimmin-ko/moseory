	...
	<div class="container" style="margin-left: 22%;">
		...
		<div class="row qna_body">
			...
			<div class="col-md-5 col-md-offset-1 search_area">
				<form id="searchForm" action="/qna/qnaList" method="get">
					<select class="form-control search_type" name="type">
						<option value="T" <c:out value="${qnaPageMaker.cri.type eq 'T' ? 'selected' : '' }" />>제목</option>
						<option value="C" <c:out value="${qnaPageMaker.cri.type eq 'C' ? 'selected' : '' }" />>내용</option>
						<option value="TC" <c:out value="${qnaPageMaker.cri.type eq 'TC' ? 'selected' : '' }" />>제목+내용</option>
						<option value="W" <c:out value="${qnaPageMaker.cri.type eq 'W' ? 'selected' : '' }" />>작성자</option>
					</select> 
					<input type="text" class="form-control search_keyword" name="keyword" 
						value='<c:out value="${qnaPageMaker.cri.keyword }" />' autocomplete="off" placeholder="검색어">
					<input type="hidden" name="pageNum" value="${qnaPageMaker.cri.pageNum }">
					<input type="hidden" name="amount" value="${qnaPageMaker.cri.amount }">
				...
				<nav>
					<ul class="pagination">
						<c:if test="${qnaPageMaker.prev}">
							<li class="paginate_button previous"><a href="${qnaPageMaker.startPage-1 }">&laquo;</a></li>
						</c:if>

						<c:forEach var="num" begin="${qnaPageMaker.startPage }" end="${qnaPageMaker.endPage }">
							<li class="paginate_button ${qnaPageMaker.cri.pageNum == num ? 'active' : '' }">
								<a href="${num }">${num }</a>
							</li>
						</c:forEach>

						<c:if test="${qnaPageMaker.next }">
							<li class="paginate_button next"><a href="${qnaPageMaker.endPage+1 }">&raquo;</a></li>
						</c:if>
					</ul>
				</nav>
			</div>
			...
			<form id="actionForm" action="/qna/qnaList" method="get">
				<input type="hidden" name="pageNum" value="${qnaPageMaker.cri.pageNum }">
				<input type="hidden" name="amount" value="${qnaPageMaker.cri.amount }">
				<input type="hidden" name="type" value="${qnaPageMaker.cri.type }">
				<input type="hidden" name="keyword" value="${qnaPageMaker.cri.keyword }">
			</form>
